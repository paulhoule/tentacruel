...
class Application(ManagedGridFrame):
    ...
    def __init__(self, master=None):
        ...
        self._add(PREV_BUTTON, tk.Button, text="Prev",
                  width=15, height=1, command=self.task(self.prev))
        self._add(
            PLAY_BUTTON,
            tk.Button,
            text="Play",
    ...
    def _handle_progress(self, count):
        logger.debug("In flight command count is %s", count)
        if count == 0:
            self.update_status("ok" if self._transitioning_to is None else "starting")
        else:
    ...
    def handle_now_playing_progress(self, message):
        ...
        cur_pos = to_min_sec(message["cur_pos"])
        duration = to_min_sec(message["duration"])
